--Caseta_Connect v1.61
--
local VERSION = "1.61"
-- version v1.0.0			INITIAL PUBLIC RELEASE
-- version v1.0.1			plugin crashes when processing an empty status update
-- version v1.0.2			fixed plugin crashes when discovered bridge is not accessable
-- version v1.0.3			fixed external socat executable mangled by upload processing
-- version v1.0.4			fixed plugin does not process discovered bridge when only one device on network
-- version v1.0.5			fixed plugin does not process discovered bridge when running under UI5
-- version v1.0.6			fixed option page did not allow setting poll time or debug mode
-- version v1.0.7			fixed excessive debug data with debug mode disabled
-- version v1.0.8			changed set device variables only if changed
-- version v1.0.9			fixed initial child device status not set for devices connected to a SmartBridge (non-pro)
-- version v1.0.10		added optional instant status mechanism for SmartBridge (non-pro)
-- version v1.0.11		fixed MQTT connection lost after initial keep-alive-time
-- version v1.0.14		fixed LIP messages not processed by io engine
-- version v1.0.15		changes to allow operation with openluup
-- version v1.0.16		fixed - setTarget action failed in some circumstances
-- version v1.0.17		changes to allow LIP mode operation with openluup
-- version v1.0.20		fixed LIP status processing for non-dimmer devices
-- version v1.0.21		fixed plugin crash on unprocessed LIP status response
-- version v1.0.22		fixed number of buttons not set for pico devices
-- version v1.0.23		fixed LIP response parameters not correctly decoded for certain devices
-- version v1.0.24		fixed mapping of component ids for pico remote to Vera button ids
-- version v1.0.25		fixed clarified debug ouput text - finalized remote device component mapping
-- version v1.0.30		added support for Caseta Scenes
-- version v1.0.33		added retrieve LIP integration report and correlate IDs to LEAP devices
-- version v1.0.35		fixed detect lowest pico button ID from LIP integration report
-- version v1.0.36		added support for StartRampToLevel UPnP action
-- version v1.0.37		added support for RampTime dimmer device variable
-- version v1.0.38		fixed initial device state not set correctly for devices on pro model bridges
-- version v1.0.39		fixed support for blinds
-- version v1.0.40		extend support for RampTime device variable to blinds
-- version v1.0.41		fix config processing when unsolicited device update is received during configuration retreival
-- version v1.0.42		fix command processing when unexpected command prompt received by LIP command processor
-- version v1.0.43		additional fixes to pico button mapping
-- version v1.0.44		LIP response handler optimization
-- version v1.0.45		LEAP status not processed correctly when LIP server enabled
-- version v1.0.46		icon fix for OpenLuup v7
-- version v1.0.47		fix for vera device renumbering when bridge does not return data during configuration
-- version v1.0.48		fixed device summary generated invalid html
-- version v1.0.49		fixed configuration retrieval retry mechanism did not retry on LIP integration report failure
-- version v1.0.50		fixed plugin did not automatically fall back to using LEAP server when LIP configuration was not retrieved
-- version v1.0.51		added UPnP DimUpDown command
-- version v1.0.52		added UPnP SetStepDelta, StepUp and StepDown commands
-- version v1.0.52		extended UPnP DimUpDown, SetStepDelta, StepUp and StepDown commands to work on blinds
-- version v1.0.53		fixed mDNS discover response not processed
-- version v1.0.54		fixed silent failure on openLuup platforms without socat preinstalled
-- version v1.0.55		force logging of device discovery regardless of debug mode
-- version v1.0.56		device discovery not working on VeraPlus
-- version v1.0.57		fixed - plugin stops running if no bridges found.
-- version v1.0.58		fixed - mDns doQuery throws error if no response recieved on VeraPlus
-- version v1.58			changed - version numbering changed to allow marketplace update mechanism to work
-- version v1.59			fixed - configuration not accepted if pico remote(s) not configured
-- version v1.60			fixed - lutron account signin changes - caused plugin crash if account credentials are entered
-- version v1.61			fixed - lutron mqtt broker signin changes
...
local VERA = {
	SID = {
		["LUTRON"]		= "urn:micasaverde-com:serviceId:CasetaConnect1",
		...
		["KEYPAD"]		= "urn:micasaverde-com:serviceId:SceneController1",
		["AREA"]			= "urn:micasaverde-com:serviceId:SecuritySensor1",
	},
	DEVTYPE = {
		["SWITCH"]		= {"urn:schemas-upnp-org:device:BinaryLight:1","D_BinaryLight1.xml"},
		["SCENE"]			= {"urn:schemas-caseta-com:device:CasetaScene:1","D_CasetaScene1.xml"},
		["DIMMER"]		= {"urn:schemas-upnp-org:device:DimmableLight:1","D_DimmableLight1.xml"},
		["BLIND"]			= {"urn:schemas-micasaverde-com:device:WindowCovering:1","D_WindowCovering1.xml"},
		["SHADEGRP"]	= {"urn:schemas-micasaverde-com:device:WindowCovering:1","D_WindowCovering1.xml"},
		["KEYPAD"]		= {"urn:schemas-micasaverde-com:device:SceneController:1","D_SceneController1.xml"},
		["AREA"]			= {"urn:schemas-micasaverde-com:device:MotionSensor:1","D_MotionSensor1.xml"}
	},
	DEVFILES = {
		"D_BinaryLight1.xml",
		"D_BinaryLight1.json",
		...
		"D_SceneController1.json",
		"S_SceneController1.xml"
	}
}
...
local FILE_MANIFEST = {
	FILE_LIST = {
        ["D_CasetaConnect.json"]			= "fcc7fd9ff9d52d4c494d91cccf74e905",
        ["D_CasetaConnect.xml"]				= "87a99a8a521f7d685e2630e7537ad54f",
        ["D_CasetaScene1.json"]				= "e22047338d70f58519b601ee6fb90b6b",
        ["D_CasetaScene1.xml"]				= "1829ad3d42bb1178118ad63e7e609b7f",
        ["I_CasetaConnect.xml"]				= "1e80efcd576d0e53ee067040bebea2e9",
        ["J_CasetaConnect.js"]				= "e6c2128899f6d27e485bb3683c35f945",
        ["L_Caseta_socat.mipsel"]			= "8a95f4e83737ccdff5fe3ad168c749a8",
        ["S_CasetaConnect.xml"]				= "24b5881cc107811e6752c12c17ce2cba"
	},
	...
	Validate = function(self)
		...
		if (isValid == true) then uList ="NONE," end
		local iFiles = (uList:sub(1,#uList-1) or "")
		luup.variable_set(VERA.SID["LUTRON"], "MISMATCHED_FILES", (iFiles or "NONE"), lug_device)
...
local ICONS = {
	ICON_LIST = {
		["CasetaConnect.png"] = "",
		["Caseta_Scene.png"] = ""
	},
...
local UTILITIES = {
	...
	getMiosVersion = function(self)
		...
		if (tonumber(mios_branch,10) == 1) then
			...
			elseif (tonumber(mios_major,10) == 6) then
				...
				local emulate = getVariable(VERA.SID["LUTRON"], "UI6mode", lug_device)
	...
	setStatus = function(self,message)
		...
		self:setVariable(VERA.SID["LUTRON"], "BRIDGE_STATUS", message, lug_device)
	end,
...
local CASETA = {
	...
	configureBridgeConnection = function(self,BridgeIP)
		if (self.LIP_CONNECTED == true) then
			log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Integration Server already configured.")
			return true
		end
		log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Preparing Caseta Integration support elements.")
		if (UTILITIES:file_exists("/etc/cmh-ludl/caseta_openssh_key") == false) then
			log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Creating caseta key file.",2)
...
--				os.execute("dropbearconvert openssh dropbear /etc/cmh-ludl/caseta_ssh_key /etc/cmh-ludl/caseta_key; rm /etc/cmh-ludl/caseta_ssh_key")
				...
				if (UTILITIES:file_exists("/etc/cmh-ludl/caseta_dropbear_key") == false) then
					if (PLUGIN.OPENLUUP == false) then
						log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Failed to write caseta dropbear key file.",1)
						return false, "Could not convert openssh key to dropbear format"
					else
						-- running on openluup -- assume openssh ssh client
						if (UTILITIES:file_exists("/etc/cmh-ludl/caseta_openssh_key") == false) then
							log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Failed to write caseta openssh key file.",1)
							return false, "Could not write openssh key"
						else
							log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Created caseta openssh key file.",2)
					...
					log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Created caseta dropbear key file.",2)
				end

			else
				log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Failed to create caseta keyfile.",1)
			...
			log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Using existing keyfile.")
		end
		...
		if (UTILITIES:file_exists("/etc/cmh-ludl/socat") == false) then
			if (PLUGIN.OPENLUUP == true) then
				...
				if (socat_path:gsub("\r",""):gsub("\n","") ~= "") then
					log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: linking socat executable ["..(socat_path or "NIL").."] to /etc/cmh-ludl/socat.")
					os.execute("ln -s "..socat_path.." /etc/cmh-ludl/socat")
				else
					log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: socat executable not installed.",1)
				...
				log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: creating socat executable.")
			...
			log(""..PLUGIN.NAME.."::CASETA::configureBridgeConnection: Using existing socat executable.")
		end
		return true
	end,
	DeviceTypes = {
	...
	ButtonBase = {	-- define the button id base offset (lowest button component number)
		["FourGroupRemote"] = 2,
		["Pico1Button"] = 2,
		["Pico2Button"] = 2,
	...
	ButtonMap = {	-- define the pico button component number to Vera scene button mapping)
	...
	buildDeviceSummary = function(self)
		...
		UTILITIES:setVariable(VERA.SID["LUTRON"],"DEVICE_SUMMARY",html)
	end,
	...
	processBridgeConfig = function(self)
		log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): Processing Smart Bridge configuration.")
		local bridgeType = "SmartBridge"
		-- process the servers
		if ((self.CONFIG["Servers"] == nil) or (self.CONFIG["Devices"] == nil)) then
			log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): NO CONFIGURATION to process.",1)
			return false
		end
		for idx,svr in pairs(self.CONFIG["Servers"]["Body"]["Servers"]) do
			...
			log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): Found SERVER - type ["..UTILITIES:print_r(svr["Type"]).."] EnableState ["..UTILITIES:print_r(svr["EnableState"]).."] JSON ["..(self[svr["Type"]].JSON or "NIL").."]")
		end
		...
		debug(""..PLUGIN.NAME.."::CASETA::processBridgeConfig): SmartBridge Model - raw ["..(modelString or "NIL").."] eval ["..(bridgeType or "NIL").."]")
		self.DEVICES = {}
		...
		if (self["LIP"].ENABLED and ((self.CONFIG["LIP"] == nil) or UTILITIES:arraySize(self.CONFIG["LIP"]) == 0)) then
			...
			log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): LIP Integration report not yet received. Attempting to retrieve report.",1)
			local retries = 5
			local lipReport = nil
			repeat
			...
			if ((self.CONFIG["LIP"] == nil) or (UTILITIES:arraySize(self.CONFIG["LIP"]) == 0)) then
				...
				log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): LIP Integration report not retrieved. Forcing LEAP mode.",1)
				PLUGIN.DISABLE_LIP = true
			else
				log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): LIP Integration report retrieved.",2)
			end
		end
		...
		if (self.CONFIG["LIP"] and self.CONFIG["LIP"]["Body"] and self.CONFIG["LIP"]["Body"]["LIPIdList"]) then
			...
			if (self["LIP"].ENABLED == true) then
				...
				log("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): LIP Integration report not available. Forcing LEAP mode.",1)
				PLUGIN.DISABLE_LIP = true
			end
		end

		debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig): Scanning child devices.")
		-- match reported devices to vera devices
		for idx,vDev in pairs(luup.devices) do
			if (vDev.device_num_parent == lug_device) then
				debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):  Processing device ["..(idx or "NIL").."] id ["..(vDev.id or "NIL").."].")
				...
				debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):    Scanned device ["..(idx or "NIL").."] id ["..(vDev.id or "NIL").."] - type ["..(devType or "NIL").."] num ["..(devNum or "NIL").."] zone ["..(zoneNum or "NIL").."] button ["..(buttonNum or "NIL").."].")
				if ((devType ~= nil) and (devNum ~= nil)) then
					if (devType == "SCENE") then
						...
						if (tonumber(devNum,10) == 1) then
							...
							debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):        Found Caseta scene ["..(sIdx or "NIL").."].")
							if (sIdx > 0) then
								self.SCENES[sIdx].VID = idx
								debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):        Updated Caseta scene ["..(sIdx or "NIL").."] with Vera id ["..(idx or "NIL").."].")
							end
						end
					else
						-- detect a physical device
						local dIdx = self:findDeviceIndex(devNum,zoneNum,buttonNum)
						debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):        Found Caseta device ["..(dIdx or "NIL").."].")
						if (dIdx > 0) then
							self.DEVICES[dIdx].VID = idx
							debug("("..PLUGIN.NAME.."::CASETA::processBridgeConfig):        Updated Caseta device ["..(dIdx or "NIL").."] with Vera id ["..(idx or "NIL").."].")
						end
					end
				end
			end
...
--		local self.DEVICES = Config["MultipleDeviceDefinition"]["Devices"]
	...
	getBridgeConfigLIP = function(self)
		...
		lipCFG = lipCFG:gsub("\r\n\r\n","\r\n")
		debug("(CASETA::getBridgeConfigLIP): Retrieved LIPconfig ["..(lipCFG or "NIL").."].")
	...
	getBridgeConfig = function(self)
		debug("("..PLUGIN.NAME.."::CASETA::getBridgeConfig): Retrieving config from Bridge ["..(PLUGIN.BRIDGE_IP or "NIL").."].")
		if (PLUGIN.SSH_KEYFILE == "") then 
			debug("("..PLUGIN.NAME.."::CASETA::getBridgeConfig): Plugin SSH options not configured.")
			return nil
		end
		retryCount = 5
		repeat
			self.CONFIG = nil
			debug("("..PLUGIN.NAME.."::CASETA::getBridgeConfig): Requesting configuration from bridge.",2)
...
--			local cCMD = "(sleep 2;echo '"..self.CONFIG_COMMANDS.PROJECT.."';echo '"..self.CONFIG_COMMANDS.SYSTEM.."';echo '"..self.CONFIG_COMMANDS.SERVERS.."';echo '"..self.CONFIG_COMMANDS.DEVICES.."';echo '"..self.CONFIG_COMMANDS.ZONES.."';echo '"..self.CONFIG_COMMANDS.BUTTON.."';echo '"..self.CONFIG_COMMANDS.BUTTONS.."';echo '"..self.CONFIG_COMMANDS.VIRTUAL_BUTTONS.."')|/etc/cmh-ludl/socat - EXEC:\"ssh "..PLUGIN.SSH_OPTIONS.." -i "..PLUGIN.SSH_KEYFILE.." leap@"..PLUGIN.BRIDGE_IP.."\",pty,setsid,ctty|grep -e 'ReadResponse'"
			debug("(CASETA::getBridgeConfig): Sending Command["..(cCMD or "NIL").."].")
			...
			CFG = CFG:gsub("\r\n\r\n","\r\n")
			debug("(CASETA::getBridgeConfig): Retrieved config ["..(CFG or "NIL").."].")
			self.CONFIG = nil
...
--				debug("CFG json: "..(UTILITIES:print_r(cJson)))
			...
			debug("(CASETA::getBridgeConfig): Retrieved ["..UTILITIES:arraySize(self.CONFIG).."] configuration entries.",2)
			retryCount = retryCount - 1 
		until ((UTILITIES:arraySize(self.CONFIG) > 6) or (retryCount == 0))
		if (self.CONFIG) then
			debug("(CASETA::getBridgeConfig): Retrieved config ["..UTILITIES:print_r(self.CONFIG["Devices"]).."] device entries.",2)
		else
			debug("(CASETA::getBridgeConfig): Bridge config not retrieved.",1)
		end
		return self.CONFIG
	end,
	appendDevices = function(self,device)
		log("("..PLUGIN.NAME.."::CASETA::appendDevices): Preparing for update/append of Vera devices...",2)
		local added = false
		local veraDevices = {}
		-- add/update devices - cache the scan results before committing in case of error
		for idx, dev in pairs(self.DEVICES) do
			debug("("..PLUGIN.NAME.."::CASETA::appendDevices):   Processing device ["..(dev.ID or "NIL").."] type ["..(dev.TYPE or "NIL").."]")
			local devId = "Caseta_"..dev.TYPE.."_"..dev.ID.."_"..dev.ZONE.."_"..dev.BUTTON
			if (tonumber(dev.ID,10) > 1) then
				if (VERA.DEVTYPE[dev.TYPE] ~= nil) then
					local devParams = ""
					if (dev.TYPE == "KEYPAD") then
						devParams =	"urn:micasaverde-com:serviceId:SceneController1,NumButtons="..(dev.NUM_BUTTONS and tostring((tonumber(dev.NUM_BUTTONS,10) * 2)) or "2").."\n" ..
												"urn:micasaverde-com:serviceId:SceneController1,Sl_SceneActivated=0\n" ..
												"urn:micasaverde-com:serviceId:SceneController1,LastSceneID=0\n" ..
												"urn:micasaverde-com:serviceId:SceneController1,LastSceneTime=0\n" ..
												"urn:micasaverde-com:serviceId:SceneController1,FiresOffEvents=0"
					elseif ((dev.TYPE == "DIMMER") or (dev.TYPE == "BLIND")) then
						devParams =	"urn:upnp-org:serviceId:Dimming1,RampTime=0"
					end
--					luup.chdev.append(device,ptr, devId,dev.NAME, VERA.DEVTYPE[dev.TYPE][1],VERA.DEVTYPE[dev.TYPE][2],"",devParams,false)
					veraDevices[#veraDevices+1] ={devId,dev.NAME, VERA.DEVTYPE[dev.TYPE][1],VERA.DEVTYPE[dev.TYPE][2],"",devParams,false}
					if (dev.VID == 0) then
						added = true
					else
						if (dev.TYPE == "KEYPAD") then
							-- ensure pico devices have the correct number of buttons defined
							UTILITIES:setVariable(VERA.SID["KEYPAD"],"NumButtons",(dev.NUM_BUTTONS and tostring((tonumber(dev.NUM_BUTTONS,10) * 2)) or "2"),dev.VID)
						elseif ((dev.TYPE == "DIMMER") or (dev.TYPE == "BLIND")) then
							UTILITIES:setVariableDefault(VERA.SID["DIMMER"],"RampTime",0,dev.VID)
						end
					end
					debug("("..PLUGIN.NAME.."::CASETA::appendDevices):     Processed Device ["..(idx or "NIL").."] type ["..(dev.TYPE or "NIL").."] name ["..(dev.NAME or "NIL").."] added ["..((dev.VID == 0) and "TRUE" or "FALSE").."] ")
				else
					log("("..PLUGIN.NAME.."::CASETA::appendDevices): ERROR : Unknown device type ["..(dev.TYPE or "NIL").."]!")
--					ptr = nil
					return false
				end
			end
		...
		-- add/update scenes
		for idx, scene in pairs(self.SCENES) do
			debug("("..PLUGIN.NAME.."::CASETA::appendDevices):   Processing scene ["..(scene.ID or "NIL").."] type ["..(scene.TYPE or "NIL").."]")
			local sceneId = "Caseta_"..scene.TYPE.."_1_0_"..scene.BUTTON
			if ((tonumber(scene.ID,10) > 0) and (tonumber(scene.ID,10) < 51)) then
				if (VERA.DEVTYPE[scene.TYPE] ~= nil) then
					local sceneParams =	"urn:upnp-org:serviceId:SwitchPower1,Target=0\n" .. "urn:upnp-org:serviceId:SwitchPower1,Status=0"
--					luup.chdev.append(device,ptr, sceneId, scene.NAME, VERA.DEVTYPE[scene.TYPE][1],VERA.DEVTYPE[scene.TYPE][2],"",sceneParams,false)
					veraDevices[#veraDevices+1] = {sceneId, scene.NAME, VERA.DEVTYPE[scene.TYPE][1],VERA.DEVTYPE[scene.TYPE][2],"",sceneParams,false}
					if (scene.VID == 0) then
						added = true
					end
					debug("("..PLUGIN.NAME.."::CASETA::appendDevices):     Processed Scene ["..(idx or "NIL").."] type ["..(scene.TYPE or "NIL").."] name ["..(scene.NAME or "NIL").."] added ["..((scene.VID == 0) and "TRUE" or "FALSE").."] ")
				else
					log("("..PLUGIN.NAME.."::CASETA::appendDevices): ERROR : Unknown device type ["..(dev.TYPE or "NIL").."]!")
--					ptr = nil
					return false
		...
		log("("..PLUGIN.NAME.."::CASETA::appendDevices): veraDevices count ["..#veraDevices.."] veraDevices ["..UTILITIES:print_r(veraDevices).."].",2)
		if (#veraDevices > 0) then
			log("("..PLUGIN.NAME.."::CASETA::appendDevices): Attempting to update/append Vera devices...",2)
			...
			log("("..PLUGIN.NAME.."::CASETA::appendDevices): Updated/Appended Vera devices...",2)
		else
			debug("("..PLUGIN.NAME.."::CASETA::appendDevices): Configuration error - No devices to process.",1)
			return false, false
		end

		if (added) then
			log("("..PLUGIN.NAME.."::CASETA::appendDevices): Device(s) added. RESTART pending!",1)
		else
			log("("..PLUGIN.NAME.."::CASETA::appendDevices): Device(s) updated",2)
		end
		return true, added
	end,
	CONFIG_COMMANDS = {
...
CASETA_ACTIONS = {
	...
	SetPollPeriod = function(self, lul_device, newPollTime)
		...
		UTILITIES:setVariable(VERA.SID["LUTRON"], "pollPeriod", pTime, device)
	...
	ToggleDebugMode = function(self,lul_device)
		if (PLUGIN.DEBUG_MODE == true) then
			...
			UTILITIES:setVariable(VERA.SID["LUTRON"], "DebugMode", "0", lul_device)
			UTILITIES:setVariable(VERA.SID["LUTRON"], "DebugModeText", "DISABLED", lul_device)
			task("DEBUG MODE DISABLED!",TASK.SUCCESS)
		else
			PLUGIN.DEBUG_MODE = true
			UTILITIES:setVariable(VERA.SID["LUTRON"], "DebugMode", "1", lul_device)
			UTILITIES:setVariable(VERA.SID["LUTRON"], "DebugModeText", "ENABLED", lul_device)
	...
	SetLutronUsername = function(self, lul_device, newUsername)
		...
		UTILITIES:setVariable(VERA.SID["LUTRON"], "LUTRON_USERNAME", newUsername, device)
	...
	SetLutronPassword = function(self, lul_device, newPassword)
		...
		UTILITIES:setVariable(VERA.SID["LUTRON"], "LUTRON_PASSWORD", newPassword, device)
	...
	RunLutronScene = function(self, lul_device)
		...
		local sceneId = CASETA:findSceneByVeraId(lul_device)
		if (sceneId == 0) then
	...
	sendCommandKeypad = function(self, lul_device, CommandKeypad)
		...
		UTILITIES:setVariable(VERA.SID["LUTRON"],"KeypadCommand",value,device)
		return 4,0
	end
}
...
local function checkVersion()
	local ui7Check = luup.variable_get(VERA.SID["LUTRON"], "UI7Check", lug_device) or ""

	if ui7Check == "" then
		luup.variable_set(VERA.SID["LUTRON"], "UI7Check", "false", lug_device)
		ui7Check = "false"
	end

	if( luup.version_branch == 1 and luup.version_major == 7 ) then
		luup.variable_set(VERA.SID["LUTRON"], "UI7Check", "true", lug_device)
		return true
	else
		luup.variable_set(VERA.SID["LUTRON"], "UI7Check", "false", lug_device)
		return false
...
local function getPluginSettings(device)
	...
	local plugin_version = UTILITIES:getVariable(VERA.SID["LUTRON"], "PLUGIN_VERSION")

	local period = luup.variable_get(VERA.SID["LUTRON"],"pollPeriod", device) or ""
	if period == "" then
		PLUGIN.pollPeriod = "300"
		luup.variable_set(VERA.SID["LUTRON"], "pollPeriod", PLUGIN.pollPeriod, device)
	...
	if ((plugin_version == nil) or (plugin_version == "") or (plugin_version ~= VERSION)) then
		...
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "PLUGIN_VERSION", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "BRIDGE_IP", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "BRIDGE_MAC", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "Bridge_MAC_Filter", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "MDNS_DEVICES", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "ARP_DEVICES", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "DebugMode", "0")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "DebugModeText", "DISABLED")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "BRIDGE_STATUS", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "pollPeriod", PLUGIN.pollPeriod)
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "DEVICE_SUMMARY", "")

		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "LUTRON_USERNAME", "")
		UTILITIES:setVariableDefault(VERA.SID["LUTRON"], "LUTRON_PASSWORD", "")
	...
	UTILITIES:setVariable(VERA.SID["LUTRON"], "PLUGIN_VERSION", VERSION, lug_device)
	UTILITIES:setStatus("Loading Options...")
	UTILITIES:setVariable(VERA.SID["LUTRON"], "DEVICE_SUMMARY", "", lug_device)

	if (checkVersion() == true) then
		luup.set_failure(0, lul_device)
	end

	local debugMode = luup.variable_get( VERA.SID["LUTRON"], "DebugMode", lug_device ) or ""
	if debugMode == "" then
		luup.variable_set( VERA.SID["LUTRON"], "DebugMode", (PLUGIN.DEBUG_MODE and "1" or "0"), lug_device )
	else
		PLUGIN.DEBUG_MODE = (debugMode == "1") and true or false
	end
	UTILITIES:setVariable( VERA.SID["LUTRON"], "DebugModeText", (PLUGIN.DEBUG_MODE and "ENABLED" or "DISABLED"), lug_device )
--PLUGIN.DEBUG_MODE = true
...
--------------------
function do_LIP_Login(requestType)
	if (requestType == 1) then
		debug("("..PLUGIN.NAME.."::do_LIP_Login): Received LIP login request")
		luup.io.write(PLUGIN.LIP_USERNAME.."\r\n")
	elseif (requestType == 2) then
		debug("("..PLUGIN.NAME.."::do_LIP_Login): Received LIP password request")
		luup.io.write(PLUGIN.LIP_PASSWORD.."\r\n")
		PLUGIN.LIP_LOGIN_COMPLETE = true
	end
end

function getLocalNet()
	local sCmd = ""
	if (PLUGIN.OPENLUUP) then
		sCmd = "head -n2 /proc/net/arp |tail -n1|tr -s [:blank:] ,|cut -d ',' -f1|cut -d '.' -f1-3"
	else
		if (PLUGIN.MIOS_VERSION == "UI5") then
			sCmd = "ip addr show|grep -e eth0|grep -e eth0:0|cut -d ' ' -f6|cut -d '/' -f1|cut -d '.' -f1-3"
		elseif (PLUGIN.MIOS_VERSION == "UI7") then
	...
	local LocalNet = UTILITIES:shellExecute(sCmd)
...
function load_ARP_Table(LocalNet)
	-- load the arp table using the ping command
	-- the arp table will flush unneeded entries automatically
	if ((PLUGIN.ARP_LOAD_TIMESTAMP == 0) or (os.difftime(PLUGIN.ARP_LOAD_TIMESTAMP,os.time()) > 30)) then
		debug("("..PLUGIN.NAME.."::load_ARP_Table): Loading ARP table")
		for iAddr = 1, 254 do
			sCmd = "ping -c1 -W1 "..LocalNet.."."..iAddr.."&"
			debug("("..PLUGIN.NAME.."::load_ARP_Table): sending command ["..sCmd.."]")
			-- os.execute(sCmd)
			UTILITIES:shellExecute(sCmd,false)
		end
		-- wait for the ARP table to populate
		luup.sleep(2000)
		PLUGIN.ARP_LOAD_TIMESTAMP = os.time()
	else
		debug("("..PLUGIN.NAME.."::load_ARP_Table): ARP table already loaded")
	end
	return
end

function findBridgeARP(targetMAC)
	local MAC_FILTER = ""
	local findAll = false
	if (targetMAC == nil) then
		MAC_FILTER = UTILITIES:getVariable(VERA.SID["LUTRON"],"Bridge_MAC_Filter")
		if ((MAC_FILTER == nil) or (MAC_FILTER == "")) then
			MAC_FILTER = PLUGIN.DEFAULT_MAC_FILTER
			UTILITIES:setVariable(VERA.SID["LUTRON"], "Bridge_MAC_Filter", PLUGIN.DEFAULT_MAC_FILTER)
		end
	else
		if (targetMAC == "*ALL*") then
			MAC_FILTER = ""
			findAll = true
		else
	...
	if (LocalNet == nil) then
		log("("..PLUGIN.NAME.."::findBridgeARP): could not determine the local network IP range.")
		return nil,nil,false
	end

	load_ARP_Table(LocalNet)

	log("("..PLUGIN.NAME.."::findBridgeARP): ARP table loaded")
	sCmd = "cat /proc/net/arp|grep -E '"..MAC_FILTER.."'|tr -s ' ' ,|cut -d ',' -f1,4"
	BridgeAddr = UTILITIES:shellExecute(sCmd)
	log("("..PLUGIN.NAME.."::findBridgeARP): BridgeAddr ["..(BridgeAddr or "NIL").."]")
	local bList = {}
	local sList = {}
	local fSocket = nil
	if (findAll) then
		sList[#sList+1] = "0.0.0.0,00:00:00:00:00:00,FINDALL"
		fSocket = require("socket")
	end
	if ((BridgeAddr ~= nil) and (BridgeAddr ~= "")) then
		-- convert returned data into a table
		for line in BridgeAddr:gmatch("(.-)\n") do
			line = line:gsub("\r",""):gsub("\n","")
			if (line ~= "") then
				local sLine = UTILITIES:string_split(line,",")
				if ((sLine[2] ~= "00:00:00:00:00:00") and (sLine[2]:lower() ~= "type"))then	-- filter incomplete arp entries
--					local dName = (findAll == false) and ("Lutron-"..sLine[2]..".local") or "Unknown Device"
					local dName = "Lutron-"..sLine[2]..".local"
					if (findAll) then
						dName,err = fSocket.dns.tohostname(sLine[1])
						if (dName == nil) then
							dName = "Unknown Device"
--							log("Device "..sLine[1].." ERROR INFO: "..UTILITIES:print_r(err))
						else
--							log("Device "..sLine[1].." HOST INFO: "..UTILITIES:print_r(err))
						end
					end
 					sList[#sList+1] = sLine[1]..","..sLine[2]..","..dName
					bList[#bList+1] = {
						IP = sLine[1],
						MAC = sLine[2],
						NAME = dName
	...
	if (#bList == 0) then
		log("("..PLUGIN.NAME.."::findBridgeARP): Autodiscovery did not find any Caseta SmartBridge devices",1)
		return nil,nil,false
	elseif (#bList == 1) then
		local IP,MAC = bList[1].IP, bList[1].MAC
		log("("..PLUGIN.NAME.."::findBridgeARP): Autodiscovery found possible Caseta SmartBridge) IP ["..(IP or "NIL").."] MAC ["..(MAC or "NIL").."]",2)
		return IP, MAC,false
	else
		UTILITIES:setVariable(VERA.SID["LUTRON"], "ARP_DEVICES", table.concat(sList,";"))
		if (findAll) then
			log("("..PLUGIN.NAME.."::findBridgeARP): Multiple network devices found - user intervention required",1)
		else
			log("("..PLUGIN.NAME.."::findBridgeARP): Multiple Caseta SmartBridge devices found - user intervention required",1)
		end
		return nil,nil,true
	end
end

function findBridgeMDNS()
	log("("..PLUGIN.NAME.."::findBridgeMDNS): Starting Caseta SmartBridge mDNS discovery",2)
	if (PLUGIN.DISABLE_MDNS == true) then
		debug("("..PLUGIN.NAME.."::findBridgeMDNS): mDNS discovery disabled",1)
		return nil,nil,false
	end
	MDNS:OPEN()
	local res, dPackets = MDNS:doQuery("_lutron._tcp.local")
	MDNS:CLOSE()
	if (res == false) then
		log("("..PLUGIN.NAME.."::findBridgeMDNS): mDNS Query error ["..(dPackets or "NIL").."].",1)
		return nil, nil, false
	end
	if ((dPackets == nil) or (#dPackets == 0)) then
		log("("..PLUGIN.NAME.."::findBridgeMDNS): No mDNS response received",1)
		return nil, nil, false
	end
	local discovered = MDNS:decodePackets(dPackets)
	if (#discovered > 1) then
		-- more than one bridge found - user intervention required
		-- create a device list to allow the UI to provide device selection
		local bList = {}
		for _,resp in pairs(discovered) do
			if (resp.numADDITIONAL > 0) then
				local aRecord = MDNS:getRecordsByType(resp.ADDITIONAL,"A")
				if (aRecord and (#aRecord == 1)) then
					bList[#bList+1] = aRecord[1].ADDRESS..","..UTILITIES:string_split(aRecord[1].NAME:gsub("\.local",""),"-")[2]..","..aRecord[1].NAME
				end
			end
		end
		if (#bList > 0) then
			log("("..PLUGIN.NAME.."::findBridgeMDNS): mDNS discovery found multiple Caseta SmartBridge devices... user intervention required.\nDEVICES: [\n"..UTILITIES:print_r(UTILITIES:encode_json(bList)).."\n]",1)
			UTILITIES:setVariable(VERA.SID["LUTRON"], "MDNS_DEVICES", table.concat(bList,";"))
			return nil,nil,true
		end
	else
		if (discovered[1].numADDITIONAL > 0) then
			local aRecord = MDNS:getRecordsByType(discovered[1].ADDITIONAL,"A")
			if (aRecord and (#aRecord == 1)) then
				local aIP = aRecord[1].ADDRESS
				local aMAC = UTILITIES:string_split(aRecord[1].NAME:gsub("\.local",""),"-")[2]
				log("("..PLUGIN.NAME.."::findBridgeMDNS): mDNS discovery found Caseta SmartBridge ip ["..(aIP or "NIL").."] mac ["..(aMAC or "NIL").."]",2)
				return aIP, aMAC, false
			end
		end
	end
	log("("..PLUGIN.NAME.."::findBridgeMDNS): mDNS discovery failed to find Caseta SmartBridge",1)
	return nil,nil,false
end

function findBridgeFromAccount()
	-- try to find the bridge using the serial number returned from the Lutron Account server
	log("("..PLUGIN.NAME.."::findBridgeFromAccount): Attempting to find Caseta Smart Bridge using Account info...",2)
	local res,err,params = Get_MQTT_Parameters()
	if ((res == false) and ((err == "No Lutron username") or (err == "No Lutron password"))) then
		log("("..PLUGIN.NAME.."::findBridgeFromAccount): No Lutron Account info entered...",2)
		return nil,nil,nil
	end
	if ((params.serial == nil) or (params.serial == "") or (#params.serial ~= 12)) then
		log("("..PLUGIN.NAME.."::findBridgeFromAccount): No Bridge serial returned by server...",2)
		return nil,nil,nil
	end
	PLUGIN.mqttParameters = params
	sMAC = params.serial:sub(1,2)..":"..params.serial:sub(3,4)..":"..params.serial:sub(5,6)..":"..params.serial:sub(7,8)..":"..params.serial:sub(9,10)..":"..params.serial:sub(11,12)
	local mIP,mMAC = findBridgeARP(sMac)
	if ((mIP ~= nil) and (mMAC ~= nil)) then
		log("("..PLUGIN.NAME.."::findBridgeFromAccount): Found bridge ["..(sMAC or "NIL").."] reported by account on network...",2)
		return mIP, mMAC, false
	end
	log("("..PLUGIN.NAME.."::findBridgeFromAccount): Failed to find bridge ["..(sMAC or "NIL").."] reported by account on network...",1)
	return nil, nil, false
end

function findBridge()
	-- only one bridge is supported
	log("("..PLUGIN.NAME.."::findBridge): Attempting to find Caseta Smart Bridge...",2)
	UTILITIES:setStatus("Finding Bridge...")
	local BRIDGE_IP = UTILITIES:getVariable(VERA.SID["LUTRON"],"BRIDGE_IP")
	local BRIDGE_MAC = UTILITIES:getVariable(VERA.SID["LUTRON"],"BRIDGE_MAC")
	if ((BRIDGE_IP ~= nil) and (BRIDGE_IP ~= "") and (BRIDGE_MAC ~= nil) and (BRIDGE_MAC ~= "")) then
		log("("..PLUGIN.NAME.."::findBridge): Using previously configured Caseta Smart Bridge...",2)
		return BRIDGE_IP, BRIDGE_MAC
	elseif ((BRIDGE_IP ~= nil) and (BRIDGE_IP ~= "")) then
		log("("..PLUGIN.NAME.."::findBridge): Using previously configured Caseta Smart Bridge...",2)
		return BRIDGE_IP, ""
	end
	-- try to find the bridge using Lutron Account
	local mIP, mMAC, _ = findBridgeFromAccount()
	if ((mIP == nil) and (mMAC == nil)) then
		-- find a bridge using MDNS
		mIP, mMAC, mDns_Multiple = findBridgeMDNS()
		if ((mIP == nil) and (mMAC == nil)) then
			-- mDns failed to find the bridge - do it the hard way
			mIP, mMAC, arp_Multiple = findBridgeARP()
			if ((mIP == nil) and (mMAC == nil)) then
				-- could not find a single smartbridge
				if ((mDns_Multiple == true) or (arp_Multiple == true)) then
					log("("..PLUGIN.NAME.."::findBridge): Multiple Caseta Smart Bridge devices FOUND. User intervention required...",1)
					return nil, nil, true
				else
					log("("..PLUGIN.NAME.."::findBridge): Caseta Smart Bridge NOT FOUND...",1)
					log("("..PLUGIN.NAME.."::findBridge): Loading list of network devices...")
					-- get list of network devices
					local allFound = false
					_,_,allFound = findBridgeARP("*ALL*")
					if (allFound) then
						log("("..PLUGIN.NAME.."::findBridge): Loaded list of network devices...",2)
					else
						log("("..PLUGIN.NAME.."::findBridge): Failed to load list of network devices...",1)
					end
					return nil, nil, false
				end
			end
	...
	log("("..PLUGIN.NAME.."::findBridge): FOUND Caseta Smart Bridge ["..(mIP or "NIL").."]["..(mMAC or "NIL").."]...",2)
	return mIP,mMAC,false
end
...
function Get_MQTT_Parameters()
	...
	local username = UTILITIES:getVariable(VERA.SID["LUTRON"],"LUTRON_USERNAME")
	local password = UTILITIES:getVariable(VERA.SID["LUTRON"],"LUTRON_PASSWORD")
...
function Init(lul_device)
	...
	local ret, added = CASETA:appendDevices(lul_device)
	if (ret == false) then
...
function Caseta_Startup (lul_device)
	lug_device = lul_device
	log("("..PLUGIN.NAME.."::Caseta_Startup): Caseta Connect Automation Gateway v"..VERSION.." - ************** STARTING **************",2)

	UTILITIES:setStatus("Loading Options...")

	UTILITIES:getMiosVersion()
	if (PLUGIN.MIOS_VERSION == "unknown") then
		log("("..PLUGIN.NAME.."::CasetaStartup): Unsupported MIOS version - EXITING!!.",1)
	...
	log("("..PLUGIN.NAME.."::Caseta_Startup): Caseta Smart Bridge Connect Gateway - Plugin version ["..(VERSION or "NIL").."] - isDisabled ["..(isDisabled or "NIL").."] MIOS_VERSION ["..(PLUGIN.MIOS_VERSION or "NIL").."]")
	if ((isDisabled == 1) or (isDisabled == "1")) then
		log("("..PLUGIN.NAME.."::Wink_Connect_Startup):Plugin version "..(VERSION or "NIL").." - DISABLED",2)
		PLUGIN.PLUGIN_DISABLED = true
		-- mark device as disabled
		UTILITIES:setStatus("DISABLED")
		log("("..PLUGIN.NAME.."::Caseta_Startup): Marking Caseta device: "..(lul_device or "NIL").." as disabled.",2)
	...
	PLUGIN.BRIDGE_IP, PLUGIN.BRIDGE_MAC, mFOUND = findBridge()
--	PLUGIN.DEVICE_LIST = UTILITIES:getVariable(VERA.SID["LUTRON"],"DeviceList") or ""
	if ((PLUGIN.BRIDGE_IP ~= nil) and (PLUGIN.BRIDGE_IP ~= "")) then
		debug("("..PLUGIN.NAME.."::Startup): found Caseta SmartBridge ["..(PLUGIN.BRIDGE_IP or "NIL").."] ["..(PLUGIN.BRIDGE_MAC or "NIL").."]")
		-- save the Bridge IP address
		UTILITIES:setVariable(VERA.SID["LUTRON"],"BRIDGE_IP",PLUGIN.BRIDGE_IP)
		UTILITIES:setVariable(VERA.SID["LUTRON"],"BRIDGE_MAC",PLUGIN.BRIDGE_MAC)
		local prep_resp, prep_msg = CASETA:configureBridgeConnection(PLUGIN.BRIDGE_IP)
		if (prep_resp == true) then
			if (#CASETA.DEVICES == 0) then
				-- get the bridge configuration (project definition, servers definition, devices definition, zone definition, buttons definition, virtualbuttons definition)
				debug("("..PLUGIN.NAME.."::Startup): Retrieving SmartBridge Configuration")
				UTILITIES:setStatus("Retrieving Bridge Config...")
				local Config = CASETA:getBridgeConfig()
				log("("..PLUGIN.NAME.."::Startup): getBridgeConfig returned "..(UTILITIES:arraySize(Config) or "NIL").." entries.",2)
--				if ((Config ~= nil) and (Config ~= {}) and (UTILITIES:arraySize(Config) > 6)) then	-- 7 config entries for standard bridge, 8 for pro
				if (
					(Config ~= nil) and 
					(Config ~= {}) and 
					(UTILITIES:arraySize(Config["Project"]) > 0) and
					(UTILITIES:arraySize(Config["System"]) > 0) and
					(UTILITIES:arraySize(Config["Servers"]) > 0) and
					(UTILITIES:arraySize(Config["Devices"]) > 0)
					) then	-- minimum config is Project, System, Servers and Devices
					debug("("..PLUGIN.NAME.."::Startup): found Config ["..UTILITIES:print_r(Config).."]")
					CASETA:processBridgeConfig()
					debug("("..PLUGIN.NAME.."::Startup): found Devices [\n"..UTILITIES:print_r(CASETA.DEVICES).."]\nScenes ["..UTILITIES:print_r(CASETA.SCENES).."]")
					...
					if (ret == true) then
						...
						CASETA:buildDeviceSummary()
						task ("Could not initialize plugin.", TASK.ERROR_PERM)
					end
					return true, msg, modname
				else
					UTILITIES:setStatus("Failed to load bridge config")
					-- build device summary to display installation errors
					CASETA:buildDeviceSummary()
					log("("..PLUGIN.NAME.."::Startup): Could not Retrieve minimum required SmartBridge Configuration",1)
					task ("Counld not retrieve Bridge configuration.", TASK.ERROR_PERM)
					return true, "Counld not retrieve Bridge configuration.", PLUGIN.NAME
				end
			end
		else
			log("("..PLUGIN.NAME.."::Startup): Could not configure SmartBridge connection."..(prep_msg or ""),1)
			task ("Counld not configure bridge connection.", TASK.ERROR_PERM)
			return true, "Counld not configure bridge connection.", PLUGIN.NAME
		end
	else
		-- no bridge found
		if (mFOUND == true) then
			UTILITIES:setStatus("User Intervention Required...")
			log("("..PLUGIN.NAME.."::Startup): Multiple Smart Bridge devices found. User intervention required.",1)
			-- build device summary to display installation errors
			CASETA:buildDeviceSummary()
			return true, "Select Smart Bridge Device in settings.", PLUGIN.NAME
		else
			UTILITIES:setStatus("No Bridge Found")
			-- build device summary to display installation errors
			CASETA:buildDeviceSummary()
			log("("..PLUGIN.NAME.."::Startup): Smart Bridge not found.",1)
			task ("No Smart Bridge Device found.", TASK.ERROR_PERM)
			return true, "No Smart Bridge Device found.", PLUGIN.NAME
		end
	end
	-- Callbacks
	--	getStatus = casetaPlugin.getStatus
	--	getOccupancyStatus = casetaPlugin.getOccupancyStatus
	--	return casetaPlugin.Init( lul_device )
	UTILITIES:setStatus("Implimentation Error...")
	task ("Implimentation error.", TASK.ERROR_PERM)
	return true, "Implimentation error", PLUGIN.NAME
end
