...
local VERA = {
	...
	DEVFILES = { -- TODO clean based on the files we really need
...
local FILE_MANIFEST = { -- TODO compute the md5 of these files
	FILE_LIST = {
...
local ICONS = {
	ICON_LIST = { -- TODO have proper icons for the fans
...
function findBridgeARP(targetMAC)
	...
	if (targetMAC == nil) then
		MAC_FILTER = UTILITIES:getVariable(VERA.SID["SENSEME"],"Bridge_MAC_Filter")
		if ((MAC_FILTER == nil) or (MAC_FILTER == "")) then
			MAC_FILTER = PLUGIN.DEFAULT_MAC_FILTER
			UTILITIES:setVariable(VERA.SID["SENSEME"], "Bridge_MAC_Filter", PLUGIN.DEFAULT_MAC_FILTER)
		end
	else
...
--							log("Device "..sLine[1].." HOST INFO: "..UTILITIES:print_r(err))
	...
	elseif (#bList == 1) then
		...
		UTILITIES:setVariable(VERA.SID["SENSEME"], "ARP_DEVICES", table.concat(sList,";"))
		if (findAll) then
...
function findBridgeMDNS()
	...
	if (#discovered > 1) then
		...
		if (#bList > 0) then
			...
			UTILITIES:setVariable(VERA.SID["SENSEME"], "MDNS_DEVICES", table.concat(bList,";"))
			return nil,nil,true
		end
	else
...
function findBridge()
	...
	local BRIDGE_IP = UTILITIES:getVariable(VERA.SID["SENSEME"],"BRIDGE_IP")
	local BRIDGE_MAC = UTILITIES:getVariable(VERA.SID["SENSEME"],"BRIDGE_MAC")
